...
class MultilayerPerceptron:
	...
	def __set_data_and_variables(self):
		...
		self.__ep1 = np.random.uniform(-0.001, 0.001, size=(self.__n_hidden1, self.__n_inputs))
		self.__eu1 = np.random.uniform(-0.001, 0.001, size=(self.__n_hidden1, 1))
		if self.__n_layers == 2:
			...
			self.__ep2 = np.random.uniform(-0.001, 0.001, size=(self.__n_hidden2, self.__n_hidden1))
			self.__eu2 = np.random.uniform(-0.001, 0.001, size=(self.__n_hidden2, 1))
		self.__eu3 = np.random.uniform(-0.001, 0.001, size=(self.__n_exit, 1))
	...
	def train_net(self, graph_error):
		n_epochs = self.__n_epochs_train
		done = True
		self.__fac_ap = -0.3
		self.__precision = 0.00000001
		while(n_epochs < self.__epochs):
	...
	# función principal para el entrenamiento de la red
	def train_net_quick(self, graph_error):
		n_epochs = 0
		done = True
		while(np.abs(self.__net_error) > self.__precision):
			...
			if n_epochs > self.__epochs:
				...
				break
		self.__n_epochs_train = n_epochs
		print(f'Error alcanzado con quickpropagation: {self.__net_error}')
		print(f'Número de epocas requeirdo: {n_epochs}')
		os.system('pause')
		self.train_net(graph_error)
		return done
	...
	# función que nos retorna los valores del error obtenidos por cada capa
	def __return_values_quickpropagation(self, type):
		match type:
			case 1:
				E3 = self.__s3 * np.transpose(self.__a1)
				E1 = self.__s1 * self.__Inputs
				return E3, E1
			case 2:
				E3 = self.__s3 * np.transpose(self.__a2)
				E2 = self.__s2 * np.transpose(self.__a1)
				E1 = self.__s1 * self.__Inputs
				return E3, E2, E1


	# función que retorna el incremento de w en base al algoritmo de quickpropagation
	def __return_increment_weigths(self, w, E, E_prev):
		wI = (E / (E_prev - E))
		wI = wI * w
		return wI
	...
	# Función secundaria que modifca los pesos en base a las capas de salida
	def __update_weigths_quick(self):
		match self.__n_layers:
			case 1:
				# caso de una sola capa oculta
				# actualización de los pesos de la capa de salida
				self.__s1 = self.__dtanh(self.__n1) * np.dot(np.transpose(self.__w3),self.__s3)# calculo de s2
				E3, E1 = self.__return_values_quickpropagation(1)
				self.__w3 = self.__w3 + self.__fac_ap * self.__return_increment_weigths(self.__w3, E3, self.__ep3)
				return E1
			...
			case 2:
				...
				self.__s2 = self.__dtanh(self.__n2) * np.transpose(self.__w3) * self.__s3# calculo de s2
				# Calcular sensibilidad neuronas capa oculta numero 1
				self.__s1 = self.__dtanh(self.__n1) * np.dot(np.transpose(self.__w2), self.__s2)# calculo de s1

				# calculo de las gradientes quickpropagation
				E3, E2, E1 = self.__return_values_quickpropagation(2)
				self.__w3 = self.__w3 + self.__fac_ap * self.__return_increment_weigths(self.__w3, E3, self.__ep3)

		        # Ajustar los pesos w2
				for j in range(self.__n_hidden2):
					self.__w2[j,:] = self.__w2[j,:] +  self.__fac_ap * self.__return_increment_weigths(self.__w2[j,:], E2[j,:], self.__ep2[j,:])
		        # Ajustar el umbral en las neuronas de la capa oculta numero 2
				for g in range(self.__n_hidden2):
					self.__uoc2[g,:] = self.__uoc2[g,:] + self.__fac_ap * self.__return_increment_weigths(self.__uoc2[g,:], self.__s2[g,:], self.__eu2[g,:])
				return E1

	# función principal de backpropagation
	def __quickPropagation(self):
		self.__real_error = (self.__di - self.__a3)
    	# calculo de la sensibilidad de la capa de salida
		self.__s3 = -2 *(self.__dtanh(self.__n3) * self.__real_error)
		E1 = self.__update_weigths_quick()
    
    	# Ajustar umbral us (w0)
		self.__us = (self.__us ) + self.__fac_ap * self.__return_increment_weigths(self.__us, self.__s3, self.__eu3)

        # Ajustar los pesos w1
		for j in range(self.__n_hidden1):
			self.__w1[j,:] = self.__w1[j,:] + self.__fac_ap * self.__return_increment_weigths(self.__w1[j,:], E1[j,:], self.__ep1[j,:])
        # Ajustar el umbral en las neuronas de la capa oculta 1
		for g in range(self.__n_hidden1):
			self.__uoc[g,:] = self.__uoc[g,:] + self.__fac_ap * self.__return_increment_weigths(self.__uoc[g,:], self.__s1[g,:], self.__eu1[g,:])

		if self.__n_layers > 1:
			self.__ep3, self.__ep2, self.__ep1 = self.__return_values_quickpropagation(self.__n_layers)
		else:
			self.__ep3, self.__ep1 = self.__return_values_quickpropagation(self.__n_layers)
		self.__eu3 = self.__s3.copy()
		if self.__n_layers > 1:
			self.__eu2 = self.__s2.copy()
		self.__eu1 = self.__s1.copy()
